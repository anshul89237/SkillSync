...
const Sidebar = ({ role, isCollapsed, setIsCollapsed }: SidebarProps) => {
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen bg-gradient-to-b from-[#1e1b4b] to-[#0f172a] flex flex-col justify-between z-40 transition-all duration-300 shadow-[4px_0_24px_rgba(0,0,0,0.25)] ${isCollapsed ? 'w-20' : 'w-20 lg:w-64'}`}>
      
      {/* Collapse Toggle Button - Always visible on desktop */}
      <button 
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="hidden lg:flex absolute -right-3 top-6 w-6 h-6 bg-indigo-500 rounded-full text-white items-center justify-center shadow-md hover:bg-indigo-400 z-50 transition-colors"
      >
      ...
      <div className="flex flex-col flex-1 overflow-y-auto w-full scrollbar-hide">
        {/* Brand */}
        <div className={`flex items-center justify-center ${isCollapsed ? '' : 'lg:justify-start lg:px-6'} h-20 shrink-0 border-b border-white/10`}>
      ...
      {/* Bottom Actions */}
      <div className={`w-full shrink-0 p-2 ${isCollapsed ? 'p-2' : 'lg:p-3'} border-t border-white/10 flex flex-col gap-1.5`}>
        {role === 'ROLE_LEARNER' && (
          <button
        ...
        <Link to="/help" className={`w-full flex items-center justify-center ${isCollapsed ? '' : 'lg:justify-start lg:px-4'} px-3 h-11 rounded-xl text-indigo-300/70 hover:bg-white/8 hover:text-white transition-all duration-200 group`} title={isCollapsed ? 'Help Center' : undefined}>
        ...
        <button
          ...
          className={`w-full flex items-center justify-center ${isCollapsed ? '' : 'lg:justify-start lg:px-4'} px-3 h-11 rounded-xl text-red-400/80 hover:bg-red-500/15 hover:text-red-400 transition-all duration-200 group`}
